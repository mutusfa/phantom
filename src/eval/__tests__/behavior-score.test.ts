...
describe("scoreBehavior", () => {
	describe("completionRate", () => {
		...
		test("1.0 when all labeled sessions are thumbs up", () => {
			const score = scoreBehavior(baseMetrics({ thumbsUpCount: 5, labeledSessionCount: 5, totalSessionCount: 5 }));
			expect(score.completionRate).toBe(1.0);
		});

		test("0.0 when no thumbs up despite labeled sessions", () => {
			const score = scoreBehavior(baseMetrics({ thumbsUpCount: 0, labeledSessionCount: 4, totalSessionCount: 4 }));
			expect(score.completionRate).toBe(0.0);
		});

		test("0.5 with half thumbs up", () => {
			const score = scoreBehavior(baseMetrics({ thumbsUpCount: 3, labeledSessionCount: 6, totalSessionCount: 6 }));
	...
	describe("interventionRate", () => {
		...
		test("corrections count double confirmations", () => {
			...
			const score = scoreBehavior(baseMetrics({ correctionCount: 2, confirmationCount: 2, totalSessionCount: 5 }));
		...
		test("score is 0 when weighted rate exceeds max", () => {
			...
			const score = scoreBehavior(baseMetrics({ correctionCount: 8, confirmationCount: 0, totalSessionCount: 2 }));
	...
	describe("costScore", () => {
		...
		test("1.0 when cost is half the baseline", () => {
			const score = scoreBehavior(baseMetrics({ avgCostUsd: 0.005, baselineCostUsd: 0.01 }));
			// ratio = 0.5, score = 1.5 - 0.5 = 1.0
			expect(score.costScore).toBeCloseTo(1.0);
		});

		test("0.5 when cost equals baseline", () => {
			const score = scoreBehavior(baseMetrics({ avgCostUsd: 0.01, baselineCostUsd: 0.01 }));
			// ratio = 1.0, score = 1.5 - 1.0 = 0.5
			expect(score.costScore).toBeCloseTo(0.5);
		});

		test("0 when cost is 50% above baseline", () => {
			const score = scoreBehavior(baseMetrics({ avgCostUsd: 0.015, baselineCostUsd: 0.01 }));
	...
	describe("verbosityScore", () => {
		...
		test("1.0 when output is half the baseline", () => {
			const score = scoreBehavior(baseMetrics({ avgOutputTokens: 500, baselineOutputTokens: 1000 }));
